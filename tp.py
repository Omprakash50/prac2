...
cdef class CrossExchangeMarketMakingStrategy(StrategyBase):
    ...
    def __init__(self,
        ...
        self.logger().warning(f"{self._market_pairs}  ooooooooooo {self._maker_markets} ooooooooooo  {self._taker_markets} ooooooooooo  {self._maker_order_ids}")
    ...
    def format_status(self) -> str:
        ...
        for market_pair in self._market_pairs.values():
            ...
            if market_pair in tracked_maker_orders and len(tracked_maker_orders[market_pair]) > 0:
                ...
                bid = self.c_get_top_bid_ask(market_pair)
                mid_price = bid
            ...
            warning_lines.extend(self.balance_warning([market_pair.maker]))
    ...
    cdef c_process_market_pair(self, object market_pair, list active_orders):
        ...
        self.c_check_and_create_new_orders(market_pair, has_active_bid)
    ...
    cdef object c_get_market_making_price(self,
        ...
        top_bid_price = self.c_get_top_bid_ask_from_price_samples(market_pair)
    ...
    cdef c_get_suggested_price_samples(self, object market_pair):
        """
    ...
    cdef c_get_top_bid_ask(self, object market_pair):
        """
    ...
    cdef c_take_suggested_price_sample(self, object market_pair):
        ...
        if ((self._last_timestamp // self.ORDER_ADJUST_SAMPLE_INTERVAL) <
            ...
            if market_pair not in self._suggested_price_samples:
                self._suggested_price_samples[market_pair] = (deque())

            top_bid_price = self.c_get_top_bid_ask_from_price_samples(market_pair)

            bid_price_samples_deque= self._suggested_price_samples[market_pair]
    ...
    cdef c_get_top_bid_ask_from_price_samples(self,
                                                    object market_pair):
        """
        ...
        current_top_bid_price = self.c_get_top_bid_ask(market_pair)

        bid_price_samples = self.c_get_suggested_price_samples(market_pair)
        self.logger().warning(f"{current_top_bid_price}  ooooooooooo {bid_price_samples}")
    ...
    cdef c_check_and_create_new_orders(self, object market_pair, bint has_active_bid):
        """
